...
if(isset($_SESSION['admn']) && isset($_GET['eid']))
{
	unset($_SESSION['empid']);
	$_SESSION['empid']=trim($_GET['eid']);
	$empid=trim($_GET['eid']);
	$name_eng=trim($_GET['enm']);
...
class myPDF extends FPDF{
	...
	function personal(){
		$this->Image('rakub.png',75,7,15,15);
...
$pdf->addFont('NikoshBAN','','NikoshBAN.php');
...
$pdf->personal();
/*
$pdf->SetFont('NikoshBAN','',10);
//$str=iconv('UTF-8', 'windows-1252','আমার নাম');
//$str= iconv('utf-8', 'cp1252','আমার নাম');
$str=utf8_decode('আমার নাম');
$str= mb_convert_encoding($str,'utf-8', 'utf-8');
$pdf->Cell(200,5,$str,0,0);
*/
$data=mysqli_query($conn,'select `photo` from `personal` where `emp_id`='.$empid);
...
while($row=mysqli_fetch_array($data)){
	...
	//$str=iconv('UTF-8', 'windows-1252',$row['name_ban']);
	...
	$pdf->Row(Array('Name (In English)',':',empty($row['name_eng'])? ' ': $row['name_eng'],
					'Name (In Bengali)',':',empty($row['name_ban'])? ' ': utf8_decode($row['name_ban'])));
...
/*CASE
         WHEN examName IN ("SSC","ssc","S S C","Ssc","S.S.C") THEN 1
         WHEN examName In ("HSC","hsc","H S C","Hsc","H.S.C") THEN 2
         WHEN examName="Graduation" THEN 3
         WHEN examName="Post-Graduation" THEN 4
         WHEN examName="phd" THEN 5
         ELSE 6 END , examName';	*/
...
$pdf->Image('../photo/'.$sign['signature'],$pdf->GetX()-40,$pdf->GetY()+5,60,16);
$pdf->Ln();

$pdf->output($empid.'.pdf','I');
