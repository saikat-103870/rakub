...
$pdf->Row(Array('Contract History',':',empty($row['contractHistory'])? ' ': $row['contractHistory'],
	...
	$pdf->Ln(12);
			...
			$pdf->SetFont('Times','B','15');
			$pdf->Cell(200,5,'Permanent Address:',20,20);
			//$pdf->Ln();
			$pdf->Line(10,$pdf->GetY(),200,$pdf->GetY());
			$pdf->SetFont('Times','','24');
			$pdf->Row(Array('Street',':',empty($row['PerStreet'])? ' ': $row['PerStreet'],
						'Postal',':',empty($row['PerPostal'])? ' ': $row['PerPostal']));
			$pdf->Row(Array('District',':',empty($row['PerDistrict'])? ' ': $row['PerDistrict'],
						'Country',':',empty($row['PerCountry'])? ' ': $row['PerCountry']));
		
			$pdf->SetFont('Times','B','15');
			$pdf->Cell(200,5,'Present Address:',20,20);
			//$pdf->Ln();
			$pdf->Line(10,$pdf->GetY(),200,$pdf->GetY());
			$pdf->SetFont('Times','','24');
			$pdf->Row(Array('Street',':',empty($row['PreStreet'])? ' ': $row['PreStreet'],
						'Postal',':',empty($row['PrePostal'])? ' ': $row['PrePostal']));
			$pdf->Row(Array('District',':',empty($row['PreDistrict'])? ' ': $row['PreDistrict'],
						'Country',':',empty($row['PreCountry'])? ' ': $row['PreCountry']));
