...
//$pdf->Cell(300,5,'CIB Inquiry Form-1',0,0);
$pdf->Cell(205,5,'CIB Inquiry Form-1',0,0,'C');
$pdf->Ln(9);
...
$data=mysqli_query($conn,"SELECT distinct i.*,c.*,d.* from inquiry i INNER JOIN individual c 
...
while($row=mysqli_fetch_array($data)){
	//Personal Information	
			...
			$pdf->Row(Array('Total Requested Amount.',':',empty($row['TotalReqAmt'])? ' ': $row['TotalReqAmt'],
						'Role in the Institution',':',empty($row['Role'])? ' ': $row['Role']));
			...
			$pdf->Row(Array('Sector Type',':',empty($row['SecType'])? ' ': $row['SecType'],
				'Sector Code',':',empty($row['SecCode'])? ' ': $row['SecCode']));	

			$pdf->Ln();
			$pdf->SetFont('Times','B','15');
			$pdf->Cell(200,5,'Addresses:',20,20);
			$pdf->Line(10,$pdf->GetY(),200,$pdf->GetY());
			//$pdf->Ln();
			$pdf->Sety(141);
			$pdf->SetFont('Times','B','10');
