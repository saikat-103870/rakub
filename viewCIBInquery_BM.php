...
if(isset($_GET['gtrNo']))
{
	...
	on i.TrackingNo=c.TrackingNo where i.TrackingNo=$trNo and i.status=1";
	...
	{
		echo '<center>';
		echo '<h2><br><br><br><br><span style="color: red;">Already sent to Branch Manager.</span></h2>';
...
if (isset($_POST['SubmitZM']))
{
	...
	$sql = "UPDATE `inquiry` SET authorizedUserId='$EntryUserId', authorizedDateTime=CURRENT_TIMESTAMP,`status`=2 where `TrackingNo`='$trN'";
	//echo '<br><br><br><br>';
	//echo $sql;exit();
...
<span style="margin-left:10px;"> &nbsp; <button type="submit" id="SubmitZM" name="SubmitZM" class="btn btn-lg btn-success">Submit to Zonal Office <span class='glyphicon glyphicon-step-forward'></span></button> </span>
