...
while($row=mysqli_fetch_array($data)){
	...
	//Permanent Address
	$perAddress='House: '.(empty($row['per_house'])? '--': $row['per_house']).', Road: '.(empty($row['per_road'])? '--': $row['per_road']).', Village: '.(empty($row['per_vill'])? '--': $row['per_vill']).', Post Office: '.(empty($row['per_postoffice'])? '--': $row['per_postoffice']).', Police Station/City Corporation: '.($pdf->getPS($conn,$pdf,$row['per_div'],$row['per_dist'],empty($row['per_ps'])? ' ': $row['per_ps'],'location','UpazillaName')).', District: '.($pdf->getDistrict($conn,$pdf,empty($row['per_dist'])? ' ': $row['per_dist'],'location','DistrictCode','DistrictName')).', Division: '.($pdf->getDiv($conn,$pdf,empty($row['per_div'])? ' ': $row['per_div'],'location','DivisionName','DivisionCode')); 
...
$pdf->MultiCell(182,4.5,'আমি এই মর্মে অঙ্গীকার করিতেছি যে, উল্লিখিত বিবরণী আমার জানামতে নির্ভুল এবং সত্য। উপরোক্ত বিবরণের মধ্যে যদি কোন অসত্য অথবা অসমাপ্ত বিবরণ প্রমাণিত হয়, তবে রাজশাহী কৃষি উন্নয়ন ব্যাংকের একজন কর্মকর্তা/কর্মচারি হিসাবে ব্যাংক প্রদত্ত আইনানুগ যেকোন ব্যবস্থা মেনে নিতে বাধ্য থাকিব।',0,'J',0);
$pdf->Ln();
