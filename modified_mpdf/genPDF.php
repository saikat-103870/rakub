...
while($row=mysqli_fetch_array($data)){
	...
	$pdf->Row(Array('Approval Authority',':',empty($row['st_authority'])? ' ': $row['st_authority'],
					'Letter Reference No',':',empty($row['st_ref'])? ' ': $row['st_ref']),1);
...
$data=mysqli_query($conn,'select * from `spouse` where `emp_id`='.$empid.' order by sp_dob desc');
...
$data=mysqli_query($conn,'select * from `training` where `emp_id`='.$empid.' order by startDate desc');
...
$data=mysqli_query($conn,'select * from `lien` where `emp_id`='.$empid.' order by FromDate desc');
if (mysqli_num_rows($data) > 0) 
{
	while($row=mysqli_fetch_array($data)){
...
$data=mysqli_query($conn,'select * from `reward` where `emp_id`='.$empid.' order by Rwarddate desc');
if (mysqli_num_rows($data) > 0) 
{
	while($row=mysqli_fetch_array($data)){
...
$data=mysqli_query($conn,'select * from `punishment` where `emp_id`='.$empid.' order by p_date desc');
if (mysqli_num_rows($data) > 0) 
{
	while($row=mysqli_fetch_array($data)){
