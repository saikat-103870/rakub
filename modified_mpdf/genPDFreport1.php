...
$pdf->Ln(8);
$noEmp=$_POST['empno'];
$pdf->Cell(200,5,'List of Branches Having More Than '.$noEmp.' Employee',0,0);
$pdf->Ln(10);

$pdf->SetFont('Times','B',12);
$pdf->SetWidths(Array(70,80,30));
$pdf->Row2(Array('Zone Name','Branch Name','No. Of Employee'));
$pdf->SetFont('nikosh','',10);


$data=mysqli_query($conn,'select COUNT(DISTINCT tb.`emp_id`) numemp,tb.Zone_en,tb.brNameEn  from
(SELECT p.`prPostingZone`,z.`Zone_en`,b.`brNameEn`, p.`emp_id` FROM `personal` p 

INNER JOIN zone z on trim(z.zoneCode)=trim(p.prPostingZone)
INNER JOIN br b on trim(b.brCode)=trim(p.prPostingBr)

 ) tb
GROUP BY  Zone_en,brNameEn
HAVING numemp>'.$noEmp);
