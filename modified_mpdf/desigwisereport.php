...
if(trim($_POST['Desi'])=='0')
{
	echo '<br><br><br><br><br><br><center><div style="color: red;font-size:18px;">Please select Designation</div>';
	echo '<a href="desigwisereport.php" class="btn btn-lg">Back</a></center>';
	exit;
}
...
$mpdf->Ln(5);
$mpdf->WriteHTML('Employee List of '.$desi);
$mpdf->Ln();
...
<div class="panel-heading" style="background-color:#008836;color: white; font-size:16px;">Enter Your Designation</div>
...
<div class="form-group">
	  ...
	  <select class="form-control" id="Desi" name="Desi">
	   <option selected value="0">--Select Designation--</option>
		<?php
		 $sqlhdeg="SELECT DISTINCT `id` ,`name` FROM `emp_desg` ORDER by name asc";
		

		$eploydesig=mysqli_query($conn,$sqlhdeg);

		while($row = mysqli_fetch_assoc($eploydesig)) 
		{
		?>
		<option value="<?php echo $row["name"]; ?>" 
		<?php if($designation==trim($row["name"])) echo 'selected="selected"'; ?> ><?php echo $row["name"] ?>
		</option>	
